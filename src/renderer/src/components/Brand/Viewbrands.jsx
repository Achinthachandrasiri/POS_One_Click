...
const ViewBrands = () => {
  ...
  const { getAllBrands, deleteBrand, loading, error } = useBrandHooks()
    ...
    <div className="relative min-h-full px-8 pt-8 pb-0 overflow-hidden">
      <div className="flex flex-col h-[630px]">
        ...
        <div className="relative z-10 mb-5">
          <h1 className="text-white text-[22px] font-semibold m-0">Brands</h1>
          <p className="text-[#90bcc4] text-[15px] mt-1">Manage brand records</p>
        </div>
            ...
            <table className="w-full min-w-[500px] border-collapse text-sm">
              <thead>
                <tr className="bg-[#f5fbfd] text-[#2a5b67] text-xs">
                  ...
                  <th className="text-left px-3 py-2 border-b">Brand Name</th>
                  ...
                  filteredBrands.map((brand) => (
                    <tr key={brand._id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2">
                        {toImageSrc(brand.image) ? (
                          <img
                            src={toImageSrc(brand.image)}
                            alt={brand.brandName}
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex gap-2 justify-end w-full">
                          <button
                            onClick={() => navigate(`/dashboard/products/brands/edit/${brand._id}`)}
