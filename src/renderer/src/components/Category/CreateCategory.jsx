...
const CreateCategory = () => {
  ...
  const { createCategory, loading, error, fieldErrors, setFieldErrors } = useCategoryHooks()
  ...
  const handleSubmit = async (e) => {
    ...
    if (res?.success) {
      ...
      setForm({ categoryName: '', image: null, imagePath: '' })
      setImagePreview(null)
      if (fileInputRef.current) fileInputRef.current.value = ''
    }
  }
    ...
    <div className="relative min-h-full px-8 pt-8 pb-0 overflow-hidden">
      <div className="flex flex-col h-[630px]">
        ...
        <div className="relative z-10 mb-5">
          <h1 className="text-white text-[22px] font-semibold m-0">Create Category</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {createdCategory?.categoryName && (
              <div className="bg-gray-100 p-3 rounded text-xs">
                <p className="text-gray-600">Category Created:</p>
                <p className="font-mono text-gray-900">{createdCategory.categoryName}</p>
              </div>
            )}
