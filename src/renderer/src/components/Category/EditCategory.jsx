...
const EditCategory = () => {
  ...
  const [form, setForm] = useState({ categoryName: '', imagePath: '' })
  ...
  useEffect(() => {
    const loadCategory = async () => {
      ...
      if (res?.success && res.data) {
        ...
        if (res.data.image) {
          setImagePreview(toImageSrc(res.data.image))
        }
      }
    }
    loadCategory()
  ...
  const handleSubmit = async (e) => {
    ...
    if (res?.success) {
      ...
      setTimeout(() => navigate('/dashboard/products/categories'), 500)
    }
  }
    ...
    <div className="relative min-h-full px-8 pt-8 pb-0 overflow-hidden">
      <div className="flex flex-col h-[630px]">
        ...
        <div className="relative z-10 mb-5">
          <h1 className="text-white text-[22px] font-semibold m-0">Edit Category</h1>
          <p className="text-[#90bcc4] text-[15px] mt-1">Update category details</p>
        </div>
            ...
            <div className="flex gap-6 items-start">
              ...
              <div className="flex-1">
                ...
                {fieldErrors.categoryName && (
                  <p className="text-red-600 text-xs mt-1">{fieldErrors.categoryName}</p>
                )}
              </div>
                ...
                {!imagePreview ? (
                  ...
                  <div className="mt-2 relative w-[120px] h-[120px]">
                    ...
                    <button
                      type="button"
                      onClick={() => fileInputRef.current?.click()}
                      className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-[10px] py-1 rounded-b-lg hover:bg-opacity-70 transition"
                    >
                      Replace
                    </button>
                    <button
