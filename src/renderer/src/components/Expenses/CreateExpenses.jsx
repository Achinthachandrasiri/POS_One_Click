...
const CreateExpenses = () => {
  ...
  const { createExpense, loading } = useExpenseHooks()
  ...
  const handleSubmit = async (e) => {
    ...
    if (res?.success) {
      ...
      setFormError(res?.error || 'Failed to create expense. Please try again.')
    }
  }
    ...
    <div className="relative min-h-full px-8 pt-8 pb-0 overflow-hidden">
      <div className="flex flex-col h-[630px]">
        ...
        <div className="relative z-10 mb-5">
          <h1 className="text-white text-[22px] font-semibold m-0">Create Expense</h1>
          <p className="text-[#90bcc4] text-[15px] mt-1">Create new expense records</p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} className="max-w-2xl mt-6">
            ...
            <div className="flex gap-3 mt-7">
              ...
              >
                {loading ? (<><i className="fas fa-spinner fa-spin" /> Saving…</>) : (<><i className="fas fa-plus" /> Add Expense</>)}
              </button>
            </div>
