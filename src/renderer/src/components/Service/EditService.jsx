...
const EditService = () => {
  ...
  const { id } = useParams()
  const { updateService, getServiceById, loading } = useServiceHooks()
  ...
  const handleSubmit = async (e) => {
    ...
    if (res?.success) {
      ...
      setFormError(res?.error || 'Failed to update service. Please try again.')
    }
  }
  ...
  if (pageLoading) {
    return (
      <div className="flex items-center justify-center h-[630px] text-gray-400">
        <i className="fas fa-spinner fa-spin mr-2" />Loading service…
      </div>
    )
  }
    ...
    <div className="relative min-h-full px-8 pt-8 pb-0 overflow-hidden">
      <div className="flex flex-col h-[630px]">
        ...
        <div className="relative z-10 mb-5">
          <h1 className="text-white text-[22px] font-semibold m-0">Edit Service</h1>
          <p className="text-[#90bcc4] text-[15px] mt-1">Edit service records</p>
        </div>
